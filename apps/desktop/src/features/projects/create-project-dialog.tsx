...
import { isTauri, pickTauriProjectFolder } from "@/lib/tauri";
...
export function CreateProjectDialog({
  ...
  const pickFolder = useCallback(async () => {
    console.log("[CreateProjectDialog] pickFolder clicked, isTauri:", isTauri());
    if (!isTauri()) {
      console.warn("[CreateProjectDialog] Not running in Tauri");
      return;
    }
    try {
      const selected = await pickTauriProjectFolder();
      console.log("[CreateProjectDialog] Folder picker result:", selected);
      if (selected) {
        setPath(selected);
      }
    } catch (err) {
